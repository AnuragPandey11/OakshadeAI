// src/components/ui/flip-card.jsx
...
export default function CardFlip({
  ...
  color = "#171717",
  media, // optional GIF / image URL shown on the front instead of the code animation
}) {
  const [isFlipped, setIsFlipped] = useState(false);

  return (
    <div
      style={{ "--primary": color ?? "#171717" }}
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="from-primary/5 absolute inset-0 bg-gradient-to-br via-transparent to-slate-400/5" />

          {/* Custom GIF / image (if provided) */}
          {media && (
            <>
              <img
                src={media}
              ...
              />
              {/* Bottom scrim keeps the title/subtitle readable over the media */}
              <div className="absolute inset-x-0 bottom-0 h-2/5 bg-gradient-to-t from-white via-white/80 to-transparent" />
            </>
          )}

          {/* Animated code blocks (default when no media) */}
          {!media && (
        ...
        >
          {/* Background gradient */}
          <div className="from-primary/5 absolute inset-0 rounded-2xl bg-gradient-to-br via-transparent to-slate-400/5" />
